...
- [x] M07：计划编辑、预览、工具条、Toast 与设置状态卡
  - [x] 计划编辑器：可编辑标题、图表类型、页面方向、分组、节点和连线；节点、分组和连线 ID 由程序维护并只读展示。
  - [x] 编辑联动：删除节点时同步删除关联连线；删除分组时解除节点归属；端点、分组、类型和必填文本只在 Renderer 用户输入层校验。
  - [x] 保存语义：采用显式保存，不自动保存；保存成功后调用现有 `savePlan()` 生成新计划修订，并按 Store 既有语义使旧活动产物失效。
  - [x] 离开保护：计划存在未保存修改时接入 AppRouter 既有 leave guard；任务运行期间禁用冲突的编辑、保存和步骤切换操作。
  - [x] PNG 预览：扩展 `yibiao-asset`，新增 `visio-diagram` 资产根并映射到 `getVisioDiagramDir(app)`；相对路径逐段编码，Renderer 不读取本地文件或 Base64。
  - [x] 预览交互：结果页显示当前页大图和多页切换入口；预览读取失败不影响 VSDX 打开与重新生成，不在本模块增加图片编辑能力。
  - [x] 浮动工具条：复用 `shared/ui/FloatingToolbar`，按当前步骤提供重置、上一步/下一步、保存计划、重新生成计划、生成/重新生成 Visio、打开 VSDX。
  - [x] Toast 边界：M06 已有任务成功/失败/取消提示；本模块只补计划保存、工作区重置、组件自检/重启和预览读取失败提示，避免重复通知。
  - [x] 设置状态卡：放入“组件设置”，展示 bundled/custom、支持性、可用性、进程阶段、服务版本、工具数量和最后错误，并提供自检、重启操作。
  - [x] Runtime 配置：状态卡支持选择 bundled/custom；custom 模式可编辑 command、args、cwd，env 继续保留既有值，不在普通表单展示或改写环境变量。
  - [x] 状态同步：设置保存继续完整保留 `visio_mcp`；自检、重启和配置保存后刷新卡片，不在 Renderer 缓存第二份权威运行状态。
  - [x] 专项验证：覆盖计划编辑联动、保存修订/产物失效、预览 URL 编码、状态卡状态映射和工具条禁用规则；Electron 临时 userData 验证中文路径、多页预览和离开保护。
  - [x] 完成门槛：相关 TypeScript/CJS 检查、M01-M07 回归、Electron 冒烟和 `npm run build` 全部通过；更新本清单后独立 Git 提交。
...
- [ ] M08：Windows Sidecar 打包、Analytics 映射和端到端验收
  ...
  - [ ] M08-C：分层端到端验收
    ...
    - [ ] Visio 实机层：在安装 Microsoft Visio Desktop 的 Windows x64 上，从正式安装包完成组件自检、中文需求、计划编辑、PNG 预览、VSDX 生成/打开、重新生成和重启 Sidecar。
      ...
      - [x] 手动关闭 Visio 后重画恢复（2026-08-19）：确认上游 `VisioApp` 单例会缓存 COM Application；首次绘图后用户退出 `VISIO.EXE`，下一次 `create_diagram` 复用失效代理并返回 `-2147023174 / 0x800706BA / RPC 服务器不可用`。Renderer 仅在尚未创建文档的 `create_diagram` 命中该错误时重启 Sidecar 并重试一次，不对绘制中途操作做可能重复落图的自动重试。自动回归 21/21、bundled Sidecar 40 工具自检和 `npm run build` 通过；真实同会话执行“生成→退出 Visio→再次生成”时第二次先复现 RPC 错误、随后自动恢复，前后均生成 6 节点、6 连线、34286 bytes VSDX。
  ...
  - [ ] M08-D：大规模图表自适应布局
    - 设计基线：按 `VISIO_MCP_M08_D_DESIGN.md` 执行；本项只在动态轨道布局、Sidecar 页面适配、安装包校验和真实 Visio 大图验收全部完成后勾选。
    - [ ] 动态轨道布局：根据每行、每列的最大节点宽高和固定安全间距计算坐标；节点增多时扩展画布，不压缩节点间距。
    - [ ] 页面需求尺寸：小图保持方向对应的 A4 基础画布，大图按节点边界和边距扩展 `layout.page.width/height`。
    - [ ] 重叠诊断：纯函数覆盖节点矩形相交检查，Renderer 在调用 MCP 前记录冲突节点并阻止发布不可用产物。
    - [ ] Sidecar 页面工具：在易标本地扩展层注册 `fit_page_to_drawing`，优先调用 `Page.AutoSizeDrawing`，不可用时通过 PageSheet 尺寸确定性回退；不修改已锁定上游 wheel。
    - [ ] Renderer 接入：全部节点、连接线和连线文字完成后适应页面，再保存和导出；bundled/custom Runtime 均将新工具列为必需能力。
    - [ ] 自动验证：覆盖单行/单列大量节点、深层级、宽节点、显式行列、不同方向、小图基础尺寸、大图扩展和页面适配失败清理。
    - [ ] 打包验证：重建 Sidecar，更新 manifest/EXE 哈希，验证源码及 NSIS/ZIP/MSI 资源，macOS 继续不携带 Sidecar。
    - [ ] Visio 实机验收：30 节点流程图、40 节点组织结构图、长中文块图和深层网络图均无节点覆盖、页面裁切和残留进程，VSDX 保持可编辑。
    - [ ] 子模块提交：M08-D 代码、Sidecar、安装包和实机验收全部通过后更新验证记录、勾选本项并独立 Git 提交。
  - [ ] 完成门槛：M08-A/M08-B/M08-C/M08-D 均已勾选，Windows 安装包端到端通过，大规模图表无节点覆盖或页面裁切，Analytics 等价能力保留，M01-M08 全回归通过；最后更新 M08 总项并提交验收记录。
